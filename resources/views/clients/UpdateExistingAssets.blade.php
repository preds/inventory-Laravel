                ...
                <select class="form-control @error('designation') is-invalid @enderror" id="designation" name="designation">
                    @foreach($designations as $designation)
                        <option value="{{ $designation->id }}" 
                                data-abbreviation="{{ $designation->abbreviation_code }}"
                                {{ $asset->getDefaultDesignation() == $designation->id || $asset->getDefaultDesignation() == $designation->designation_name ? 'selected' : '' }}>
                        ...
                        <div class="row">
                            ...
                            <div class="col">
                                <div class="mb-3">
                                    ...
                                    <input class="form-control @error('bailleur') is-invalid @enderror" type="text" id="bailleur" name="bailleur" value="{{ $asset->bailleur }}">
                                    @error('bailleur')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col">
                                <div class="mb-3">
                                    ...
                                    <input class="form-control @error('projet') is-invalid @enderror" type="text" id="projet" name="projet" value="{{ $asset->projet }}">
                                    @error('projet')
                                    <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const dateActuel = new Date().toISOString().slice(0, 10);
                ...
                // Format du numéro de séquence avec 3 chiffres
                const sequence = sequenceNumber.toString().padStart(3, '0');
        
                // Création de la codification
                return `BF/${localisation}/${projet}/${bailleur}/${designation}/${dateActuel}/${sequence}`;
            }
        
            function updateCodification() {
                // Exemple de numéro de séquence, à adapter selon les besoins
                const sequenceNumber = 1; // À remplacer par la logique appropriée pour générer ou récupérer le numéro de séquence
                codificationField.value = generateCodification(sequenceNumber);
            }
            ...
            // Événements pour mettre à jour la codification lors du changement d'un champ
