                                    ...
                                    <select class="form-control @error('designation') is-invalid @enderror" id="designation" name="designation" onchange="updateCodification()">
                                        ...
                                        @foreach($designations as $designation)
                                            <option value="{{ $designation->id }}" data-abbreviation="{{ $designation->abbreviation_code }}" 
                                                @if(isset($currentDesignationId) && $designation->id == $currentDesignationId) selected @endif>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Fonction pour générer la codification sans changer le numéro de séquence s'il existe
        function generateCodification() {
            ...
            const projet = projetField.options[projetField.selectedIndex].getAttribute('data-abbreviation') || projetField.value.trim();
            const bailleur = bailleurField.options[bailleurField.selectedIndex].getAttribute('data-abbreviation') || bailleurField.value.trim();
            ...
            // Conserver la partie du numéro de séquence s'il existe dans la codification actuelle
            const currentCodification = codificationField.value;
            let sequence = '001';  // Valeur par défaut pour le numéro de séquence

            if (currentCodification) {
                // Extraction du numéro de séquence actuel (les 3 derniers caractères après le dernier '/')
                const parts = currentCodification.split('/');
                sequence = parts[parts.length - 1] || '001';
            }

            // Création de la codification
            return `BF/${localisation}/${projet}/${bailleur}/${designation}/${currentYear}/${sequence}`;
        }

        function updateCodification() {
            codificationField.value = generateCodification();
        }

        // Initialiser la codification si elle n'existe pas
        if (!codificationField.value) {
            updateCodification();
        }

        // Événements pour mettre à jour la codification lors du changement d'un champ (sauf le numéro de séquence)
        localisationField.addEventListener('change', updateCodification);
