...
<div class="container-fluid">
    <h3 class="text-dark mb-4"><span style="color: rgb(9, 179, 94);">Projets</span></h3>
    <div class="row mb-3">
        <div class="col-xxl-3">
            <div class="card shadow mb-3">
                <div class="card-header py-3">
                    <p class="text-primary m-0 fw-bold" style="color: rgb(78, 115, 223);">Ajouter un nouveau projet</p>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('projets.add') }}">
                        @csrf
                        <div class="row">
                            <div class="col">
                                <div class="mb-3">
                                    <input class="form-control @error('projet_name') is-invalid @enderror" type="text" id="projet_name" placeholder="Nom de la désignation" name="projet_name">
                                    @error('projet_name')
            ...
            <div class="card shadow mb-3">
                <div class="card-header py-3">
                    <p class="text-primary m-0 fw-bold" style="color: rgb(78, 115, 223);">Importer des projets depuis Excel</p>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('projets.import') }}" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col">
                                <div class="mb-3">
        ...
        <div class="col">
            <div class="card shadow">
                <div class="card-header py-2">
                    <p class="lead text-info m-0"><strong><span style="color: rgb(36, 117, 191);">&nbsp;</span><span style="color: rgb(18, 83, 142);">TOUS LES PROJETS</span></strong></p>

                    <div class="d-flex justify-content-between">
                        <div></div> <!-- This empty div is used to ensure the form is aligned right -->

                        <form action="/search-projets" method="GET" class="d-flex ms-auto" style="gap: 10px; align-items: center;">
                            ...
                            <select name="search_field" style="padding: 8px; border: 1px solid #ced4da; border-radius: 4px;">
                                ...
                                <option value="projet" {{ request('search_field') === 'projet' ? 'selected' : '' }}>Projets</option>
                ...
                <div class="card-body">
                            ...
                            <tbody class="contenu_tableau">
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <input type="checkbox" class="projet-checkbox" value="{{ $projet->id }}" />
                                    </td>
                                    <td class="text-center">{{ $index + 1 }}</td>
                                    <td class="text-center">{{ $projet->projet_name }}</td>
                                    <td class="text-center">{{ $projet->description }}</td>
                                    <td class="text-center">{{ $projet->abbreviation_code }}</td>
                                    <td class="text-center">
                                        <form method="POST" action="{{ route('projets.delete') }}" class="delete-form">
                                            @csrf
                                            @method('DELETE')
                        ...
                        <div class="row w-100 mt-2 mt-md-0">
                            <div class="col-md-6 align-self-center">
                                <p id="dataTable_info" class="dataTables_info" role="status" aria-live="polite">
                                    Montrer {{ $projets->firstItem() }} à {{ $projets->lastItem() }} de {{ $projets->total() }} entrées
                                </p>
                            </div>
                            <div class="col-md-6">
                                <nav class="d-lg-flex justify-content-lg-end dataTables_paginate paging_simple_numbers">
                                    {{ $projets->appends(request()->except('page'))->links('pagination::bootstrap-4') }}
                                </nav>
                            </div>
                        </div>
                    </div>
                    ...
                    <form id="delete-form" method="POST" action="{{ route('projets.delete.bulk') }}" style="display: none;">
                        @csrf
                        @method('DELETE')
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function(event) {
                ...
                const projetName = form.closest('tr').querySelector('td:nth-child(2)').textContent;

                if (confirm(`Êtes-vous sûr de vouloir supprimer la désignation "${projetName}" ?`)) {
                    form.submit();
                }
            });
        });
    ...
    const selectedIds = Array.from(document.querySelectorAll('.projet-checkbox:checked')).map(cb => cb.value);
