    ...
    public function showUpdateExistingAssetsPage(Request $request)
{
    ...
    return view('clients.UpdateExistingAssets', compact('asset', 'assets', 'categoriesInDb', 'medias'));
    ...
    public function exportLogExcel(Request $request)
{
  ...
  // Méthode pour afficher les actifs amortis
    ...
    public function showInvalidDate()
  {
      // Récupérer la date actuelle
      $currentDate = \Carbon\Carbon::now()->format('Y-m-d');

      // Récupérer les actifs avec une date d'achat invalide
      $invalidDateAssets = Asset::where(function ($query) use ($currentDate) {
          $query->whereNull('date_achat')
              ->orWhere('date_achat', '>', $currentDate);
      })->get();

      // Passer les données à la vue appropriée
      return view('clients.invalidDate', [
          'invalidDateAssets' => $invalidDateAssets,
      ]);
  }
